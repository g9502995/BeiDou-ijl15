...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::EndScene()
{
	if (!g_bImGuiInitialized)
	{
		...
		// Initialize Packet Hooks safely here
		InitPacketHooks();
	}
	...
	// Toggle logic using Insert key
	static bool showPacketLogger = true;
	static bool isInsertPressed = false;
	if (GetAsyncKeyState(VK_INSERT) & 0x8000) {
		if (!isInsertPressed) {
			showPacketLogger = !showPacketLogger;
	...
	// Force mouse cursor if menu is open
	// ImGui::GetIO().MouseDrawCursor = showPacketLogger;

	if (showPacketLogger) {
		PacketLogger::Instance().Draw(&showPacketLogger);
		// ImGui::ShowDemoWindow();
	}

	// Draw Set Item Tooltip UI
	DrawSetItemImGui();
